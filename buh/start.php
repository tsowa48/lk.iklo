        ...
        for($u = 0; $u < count($uids); ++$u) {
          ...
          for($s = 0; $s < count($schedule); ++$s) {
            if($uids[$u] === $schedule[$s]['uid'] && $curDay === $schedule[$s]['day']) {
              echo '<td style=\'padding:0px;\'><select onchange="changeTblData(\'i;'.$uids[$u].';'.$curDay.';\'+this.options[this.selectedIndex].value);" class=\'form-control\' style=\'padding-left:0px;'.
              ($isHoly ? 'background-color:pink;' : '').'\'>';
              echo '<option value=\'-1\' selected></option>';
              for($h = ((int)$v[2] === $curDay ? 6 : (((int)$v[2] + 1) === $curDay ? 0 : 6)); $h < ((int)$v[2] === $curDay ? 24 : 22); ++$h) {
                echo '<option '.($schedule[$s]['start'] === $h ? 'selected ' : '').'value=\''.$h.'\'>c '.$h.':00</option>';
              }
              echo '</select></td>';
              $isDayAdded = true;
            }
          ...
          if(!$isDayAdded) {
            ...
            echo '<option value=\'-1\' selected></option>';
            for($h = ((int)$v[2] === $curDay ? 6 : (((int)$v[2] + 1) === $curDay ? 0 : 6)); $h < ((int)$v[2] === $curDay ? 24 : 22); ++$h) {
              echo '<option value=\''.$h.'\'>c '.$h.':00</option>';
            }
            echo '</select></td>';
          }
        }
