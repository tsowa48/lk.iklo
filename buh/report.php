...
if (!isset($_SESSION['currentUser']) || !in_array(6, $_SESSION['currentUser']->post)) {
  ...
  $doys = pg_query($_SESSION['psql'], 'select doy from calendar where isholy > 0;');
  $cal = array();
  ...
  switch($id) {
    ...
    case 3:
      ...
      $result = pg_query($_SESSION['psql'], 'select U.id, U.fio, P.name, B.price, coalesce((select R.coef from rate R where R.uid=PU.uid and R.vid=B.vid), 0.0), coalesce((select sum(S.finish-S.start) from schedule S where S.uid=PU.uid and S.vid=B.vid), 0.0), U.isstaffer, U.facial from bet B, pu PU, users U, post P, komission K where U.id=PU.uid and P.id=PU.pid and B.kid=U.kid and B.pid=PU.pid and B.kid=K.id and '.($_GET['t'] === 'pred' ? 'P.name=\'Председатель\' and K.parent=' : 'K.id=').$_SESSION['currentUser']->kid.' and B.vid='.$vid.' order by P.id, U.fio;');
      while($row = pg_fetch_row($result)) {
